...
$table = str_replace(" ", "_", $tablespace);
	...
	if(mysqli_num_rows($res1) == 1)
	{
		...
		if($response != 0)
		{
			$query5 = "UPDATE $table SET response ='$response',status='3',response_time = '$resptime' WHERE question_id = '$ques_id' AND user_id = '$user_id' AND mock_id = '$mock_id'";	
			mysqli_query($con,$query5);
			
		}
		if($response == 0)
		{
			$query51 = "UPDATE $table SET response ='$response',status='2',response_time = '$resptime' WHERE question_id = '$ques_id' AND user_id = '$user_id' AND mock_id = '$mock_id'";	
		...
		else if($tita_val == 1)
		{
			if($response !== "")
		{
		$query5 = "UPDATE $table SET tita_score ='$response',status='3',response_time = '$resptime' WHERE question_id = '$ques_id' AND user_id = '$user_id' AND mock_id = '$mock_id'";	
			mysqli_query($con,$query5);	
		}
		if($response === "")
		{
		$query5 = "UPDATE $table SET  tita_score ='$response', status='2',response_time = '$resptime' WHERE question_id = '$ques_id' AND user_id = '$user_id' AND mock_id = '$mock_id'";	
			mysqli_query($con,$query5);	
		}
		}
	}
	if(mysqli_num_rows($res1) == 0)
	{
        ...
        VALUES ('$user_id','$ques_id','$mock_id','$subsection_id','$response','1','3','$resptime')";
        ...
        VALUES ('$user_id','$ques_id','$mock_id','$subsection_id','$response','1','2','$resptime')";
		 ...
		 else if($tita_val == 1)
		{
			if($response !== "")
		{
			$query62 =  "INSERT INTO $table ( user_id ,question_id,mock_id,subjectsection_id,tita_score,tita,status,response_time)
        VALUES ('$user_id','$ques_id','$mock_id','$subsection_id','$response','1','3','$resptime')";
		
		mysqli_query($con,$query62);
		}
		if($response === "")
		{
			$query62 =  "INSERT INTO $table ( user_id ,question_id,mock_id,subjectsection_id,tita,status,response_time)
        VALUES ('$user_id','$ques_id','$mock_id','$subsection_id','1','2','$resptime')";
	...
	if(mysqli_num_rows($res11) == 0)
	{
		if($tita_val1 == 0)
		{
		$query5a ="INSERT INTO $table ( user_id ,question_id,mock_id,subjectsection_id,response,options,status,response_time)
